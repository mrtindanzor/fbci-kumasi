...
function RemoveImageButton({
	...
	return (
		<Button
			...
			title="Remove image"
			className={cn(
				"z-90 absolute top-1 right-1 p-0! bg-error text-on-error hover:bg-error/90",
				className,
			)}
		>
			<X />
...
function PreviewWrapper({ className, ...props }: ComponentProps<"div">) {
	return (
		<div
			{...props}
			className={cn(
				"relative *:scale-90 w-full rounded-xl overflow-hidden bg-surface-container border border-outline-variant",
				className,
			)}
		/>
	)
